...
function GenerateQoutation({ visible, close }) {
  ...
  const HandelModelSelect = (i, element, e) => {
    ...
    preProduct[i]["details"] = { ...productDetails[element.name].products[val].data };
    preProduct[i]["width"] = productDetails[element.name].products[val].Width ? productDetails[element.name].products[val].Width.split(" ")[0] : "NA";
    preProduct[i]["length"] = productDetails[element.name].products[val].Length ? productDetails[element.name].products[val].Length.split(" ")[0] : "NA";

    preProduct[i]["wxlunit"] = preProduct[i]["width"] !== "NA" ? productDetails[element.name].products[val].Width.split(" ")[1] : preProduct[i]["length"] !== "NA" ? productDetails[element.name].products[val].Length.split(" ")[1] : "NA";

    preProduct[i]["min"] = productDetails[element.name].products[val]["Min Rate"];
    preProduct[i]["rate"] = productDetails[element.name].products[val]["Standard Rate"];
    preProduct[i]["unit"] = productDetails[element.name].products[val]["Selling Unit"];
    preProduct[i]["HSNCode"] = productDetails[element.name].products[val]["HSN Code"];
    preProduct[i]["ModleNo"] = productDetails[element.name].products[val]["ModleNo"];
    preProduct[i]["detailsTobeShown"] = { ...productDetails[element.name].products[val].data };
  ...
  const HandelSubmit = async () => {
    ...
    const requestOptions = {
      ...
      body: {
        data: { query_id: QueryId, quotation_data: JSON.stringify(pevData) }
      }
    };
                  ...
                  <div className='border-gray-100 border-2 rounded-md px-8 py-4'>
                    ...
                    <div className='flex flex-col py-2'>
                      ...
                      <select id="thikness" name="thikness" className='NewEmployeeinput' onChange={e => { HandelModelSelect(index, element, e) }} defaultValue={element.product || "Select Option"} value={element.product || "Select Option"}>
                        ...
                        {element.name ? (
                          Object.keys(productDetails[element.name].products).map((product, id) => {
                            return (
                              <option value={product} id={id}>{product}</option>
                            )
                          })
